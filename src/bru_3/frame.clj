...
(defrecord Frame [tl tr bl br]
  ...
  (edges [this]
    (let [edge (fn [[v1 v2]] (vec (map (partial get this) [v1 v2])))
          pairs [[:tl :tr] [:tr :bl] [:bl :br] [:br :tl]]]
      (map edge pairs))))

(defn from-bones
  "Creates a frame in between the two given bones b1 and b2, using the randomization maximum rm to offset the bone
  vertices. rm is given as a percentage in the [0, 1] range. When rm = 0, there'll be no vertex randomization, and the
